...
class DroneManager:
    ...
    def _mode_name(self, custom_mode) -> str:
        mode_map = self.vehicle.mode_mapping() if self.vehicle else None
        if mode_map:
            inverse = {v: k for k, v in mode_map.items()}
    ...
    def _handle_message(self, msg) -> bool:
        ...
        if mtype == "HEARTBEAT":
            # SITL では GCS 等の HEARTBEAT も届くため発生源を選別する。
            if msg.type == mavutil.mavlink.MAV_TYPE_GCS:
                return False
            if msg.autopilot == mavutil.mavlink.MAV_AUTOPILOT_INVALID:
                return False

            # 本物のオートパイロットを採用し、コマンド送信先もここに合わせる。
    ...
    def _set_mode_blocking(self, mode_name: str) -> bool:
        ...
        self.vehicle.set_mode(mode_map[mode_name])
        return True
